...
internal class ScriptTaskScope(
    ...
    /**
     * Registers a lazily-created coroutine before allowing it to execute.
     * This closes the creation/registration race with generation freeze.
     */
    internal fun <T : Job> trackAndStart(job: T): T {
        check(!job.isActive) {
            "Generation-owned coroutines must be created lazily."
        }
        track(job)
        job.start()
        return job
    ...
    internal fun cancelTrackedWork() {
        synchronized(lifecycleLock) {
            accepting = false
        }
